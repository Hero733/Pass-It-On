...
// ส่วนประวัติที่เติมให้จนสมบูรณ์
function switchHistoryTab(tab) {
    ...
    let historyItems = (tab === 'give') 
        ? posts.filter(p => p.ownerEmail === currentUser.email)
        : posts.filter(p => p.reservedByEmail === currentUser.email);

    if (historyItems.length === 0) {
        list.innerHTML = `<p class="text-center py-10 text-[#86868b] text-sm">ไม่มีประวัติในส่วนนี้</p>`;
        return;
    }

    list.innerHTML = historyItems.map(item => `
        <div class="flex items-center gap-4 bg-white/50 p-3 rounded-2xl border border-gray-100">
            <img src="${item.image}" class="w-16 h-16 object-cover rounded-xl shrink-0">
            <div class="flex-1 min-w-0">
                <p class="text-sm font-bold text-[#1d1d1f] truncate">${item.name}</p>
                <p class="text-[10px] text-[#86868b]">${item.status === 'completed' ? '✅ สำเร็จ' : (item.status === 'reserved' ? '⏳ รอส่งมอบ' : 'ว่าง')}</p>
            </div>
            ${tab === 'give' && item.status === 'reserved' ? 
                `<button onclick="completeOrder(${item.id})" class="text-[10px] bg-[#1d1d1f] text-white px-3 py-1.5 rounded-full font-bold btn-press">ยืนยัน</button>` : ''}
        </div>
    `).join('');
}
